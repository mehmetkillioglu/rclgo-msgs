...
// Do not create instances of this type directly. Always use NewPlayNext_Request
// function instead.
...
func PlayNext_Request__Sequence_to_Go(goSlice *[]PlayNext_Request, cSlice CPlayNext_Request__Sequence) {
	...
	*goSlice = make([]PlayNext_Request, int64(cSlice.size))
	for i := 0; i < int(cSlice.size); i++ {
		cIdx := (*C.rosbag2_interfaces__srv__PlayNext_Request__Sequence)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_rosbag2_interfaces__srv__PlayNext_Request * uintptr(i)),
		))
		PlayNext_RequestTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(cIdx))
	}
}
func PlayNext_Request__Sequence_to_C(cSlice *CPlayNext_Request__Sequence, goSlice []PlayNext_Request) {
	...
	cSlice.data = (*C.rosbag2_interfaces__srv__PlayNext_Request)(C.malloc((C.size_t)(C.sizeof_struct_rosbag2_interfaces__srv__PlayNext_Request * uintptr(len(goSlice)))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity

	for i, v := range goSlice {
		cIdx := (*C.rosbag2_interfaces__srv__PlayNext_Request)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_rosbag2_interfaces__srv__PlayNext_Request * uintptr(i)),
		))
		PlayNext_RequestTypeSupport.AsCStruct(unsafe.Pointer(cIdx), &v)
	}
}
